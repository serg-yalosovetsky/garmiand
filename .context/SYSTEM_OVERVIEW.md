...
- **Watch app** (`garmin/source/`)
  ...
  - `NavigationView.mc` — extends `WatchUi.View`. Owns the three map-mode rendering paths, decoded `BufferedBitmap` cache, Web Mercator tile positioning (`tileScreenRect`), and interactive zoom/pan state (`_zoomFactor`, `_interactMode`).
  - `NavigationDelegate.mc` — input: in TILES mode SELECT cycles four interact sub-modes (NS pan → WE pan → ZOOM → CENTERED → exit to NONE); UP/DOWN act according to the current sub-mode.
